...
# Panorama Stylesheet Reference

> **Verbatim registry read out of `game/bin/linuxsteamrt64/libpanorama.so`.**
> The property vocabulary comes from the CStylePropertyFactory registration; the
> prose comes from the doc strings Valve compiles into the binary — the same text
> the `dump_panorama_css_properties` console command prints.
>
> - 140 properties registered; 98 carry a description, 42 do not
> - Hard limit: 255 (the property factory index is one byte)
> - Companion doc: [Panorama CSS Quick Reference](panorama-css.md) — design-level
>   constraints and a web-CSS conversion table

Derived from game/bin/linuxsteamrt64/libpanorama.so

Everything below is read out of the binary: the property vocabulary from the
CStylePropertyFactory registration, the prose from the doc strings Valve compiles in
(the same text the `dump_panorama_css_properties` console command prints).

140 properties registered.  98 carry a description; 42 do not.
The factory index is a byte, so the vocabulary is hard-capped at 255.

NOT PRESENT, checked by name against the binary: display, flex/grid and every
companion (justify-content, align-items, gap, order, flex-*), float, clear,
top/right/bottom/left, box-sizing, content, list-style, outline, filter,
mix-blend-mode (it is -s2-mix-blend-mode), object-fit, aspect-ratio, clip-path,
mask, pointer-events, user-select, calc(), var()/custom properties, rgb()/rgba()/
hsl(), url(), and @media.  Layout is Panorama's own model instead: flow-children,
width/height taking fit-children | fill-parent-flow(w) | width-percentage(p), plus
x/y/z, align, and ignore-parent-flow.
...
@define   @import   @keyframes        (anything else: "Found unsupported CSS at-rule")

## VALUE VOCABULARY

```text
colours          #rrggbb / #rrggbbaa, and gradient(...)
gradients        the 2008 WebKit form, e.g.
...
transforms       rotate  rotate3d  rotatex  rotatey  rotatez  scale  scale3d  scalex  scaley  scalez  skew  skewx  skewy  translate  translate3d  translatex  translatey  translatez
blend modes      additive  colorburn  colordodge  darken  hardlight  hue  lighten  multiply  normal  overlay  screen  softlight
```

## PANEL XML ATTRIBUTES (layout files, not stylesheets)

acceptsfocus                    acceptsinput                    always-cache-composition-layer
analogstickscroll               childfocusonhover               class
clipaftertransform              composition-layer-texture-name  defaultfocus
disabled                        disallowedstyleflags            draggable
focusonhover                    focusonmousedown                hittest
hittestchildren                 id                              inputnamespace
keepscrolltobottom              mousecanactivate                never-cache-composition-layer
onactivate                      onblur                          oncancel
oncontextmenu                   ondblclick                      ondescendantblur
ondescendantfocus               ondeselect                      onfocus
onload                          onmouseactivate                 onmousedown
onmouseout                      onmouseover                     onmouseup
onmovedown                      onmoveleft                      onmoveright
onmoveup                        onscrolledtobottom              onscrolledtorightedge
onselect                        ontabbackward                   ontabforward
overscroll-x                    overscroll-y                    readyfordisplay
registerforreadyevents          rememberchildfocus              require-composition-layer
scrollparenttofitwhenfocused    selectionpos                    selectionposboundary
sendchildscrolledintoviewevents style                           tabindex
useglobalcontext

## Properties (all 140, verbatim from the binary)

Each entry is the exact registration and doc string from `libpanorama.so` — the same text `dump_panorama_css_properties` prints.

### `align`

(registered; Valve ships no description for it)
...
By default after slicing the image as specified in border-image-slice the 9 regions will be used to fill the space specified by the standard border-width property.  This border-image-width property may be used to override that and specify different widths.  The values appear in top, right, bottom, left order, the 2nd through 4th may be omitted and corresponding earlier values will be used.  Values may be straight floats which specify a multiple of the corresponding border-width value, a percentage (which is relative to the size of the border image in the corresponding dimension), or 'auto' which means to use the intrinsic size of the corresponding border-image-slice.
...
Sets paragraph-spacing for text in a string. Only affects multiple line breaks in a row (
...
Specifies a fixed time between frames in order to simulate a lower framerate for stylistic reasons. Default is 0s.
...
## PROPERTIES WITH NO DESCRIPTION ATTRIBUTED

Most genuinely have none. A few (visibility, background-position, letter-spacing,
paragraph-spacing, border-image-repeat) DO have prose that names them only
obliquely — those texts are in the last section rather than guessed at here.

```text
align                       animation                   animation-delay
animation-direction         animation-duration          animation-fill-mode
animation-frame-time        animation-iteration-count   animation-name
animation-timing-function   background-position         flow-children
font                        horizontal-align            ignore-parent-flow
margin                      margin-bottom               margin-left
margin-right                margin-top                  max-height
max-width                   min-height                  min-width
opacity-mask                opacity-mask-position       opacity-mask-scale
opacity-mask-threshold      padding                     padding-bottom
padding-left                padding-right               padding-top
transition-property         ui-scale-x                  ui-scale-y
ui-scale-z                  vertical-align              visibility
x                           y                           z
```

## DESCRIPTIONS THAT COULD NOT BE ATTRIBUTED TO ONE KEY

Valve wrote these; this script would have to guess which key they belong to, so it does not.

Specifies whether to short circuit applying styles when a parent is invisible.
