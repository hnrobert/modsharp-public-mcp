...
function detectLocale(filePath: string): Locale | null {
  if (filePath.includes('/en-us/') || filePath.includes('\\en-us\\'))
    return 'en';
  if (filePath.includes('/zh-cn/') || filePath.includes('\\zh-cn\\'))
    return 'cn';
  return null;
}
...
function parseTocYaml(content: string): TocNode[] {
  ...
  const hasItemsRoot = lines.some(
    (l) => /^\s*items:\s*$/.test(l) && l.search(/\S/) === 0,
  );
  if (hasItemsRoot) {
...
async function main() {
  ...
  const xmlFiles = await findFiles(join(DOCS_DIR, 'codes'), '.xml').catch(
    () => [] as string[],
  );
  ...
  // Write outputs
  await writeFile(
    join(OUTPUT_DIR, 'docs-en.json'),
    JSON.stringify(docsEn, null, 2),
  );
  await writeFile(
    join(OUTPUT_DIR, 'docs-cn.json'),
    JSON.stringify(docsCn, null, 2),
  );
  await writeFile(
    join(OUTPUT_DIR, 'examples.json'),
