...
function parseSchemaFile(filePath: string, content: string): SchemaClass | null {
  ...
  for (let i = 0; i < lines.length; i++) {
    const m = lines[i].match(/^class\s+(\w+)(?:\s*:\s*(?:public\s+)?(\w+))?\s*\{?\s*$/);
    if (m) {
      classMatchIdx = i;
      classParts = m;
      break;
  ...
  for (let i = bodyStart; i < lines.length && depth > 0; i++) {
    ...
    const typeMatch = line.match(/^\s*((?:(?:static|const|mutable)\s+)*[\w:<>,]+(?:\s*\*+)?)\s+\w+/);
    ...
    const priorityMatch = currentMeta.find((m) => m.includes("MNetworkPriority"));
    const userGroupMatch = currentMeta.find((m) => m.includes("MNetworkUserGroup") && !m.includes("Proxy"));
    ...
    const field: SchemaField = {
      ...
      type: fieldType,
      isNetworked,
      networkPriority: priorityMatch ? Number(priorityMatch.match(/=\s*(\d+)/)?.[1]) : undefined,
...
async function main(): Promise<void> {
  ...
  for (const file of files) {
    ...
    categoryCounts[schema.category] = (categoryCounts[schema.category] || 0) + 1;
  }

  await mkdir(OUTPUT_DIR, { recursive: true });
  await writeFile(join(OUTPUT_DIR, "schemas.json"), JSON.stringify(schemas, null, 2));
